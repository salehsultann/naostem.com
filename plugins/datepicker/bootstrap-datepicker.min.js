      ...
      i.startDate !== -(1 / 0) &&
        (i.startDate
          ? i.startDate instanceof Date
            ...
            : (i.startDate = g.parseDate(i.startDate, s, i.language))
          : (i.startDate = -(1 / 0))),
        i.endDate !== 1 / 0 &&
          (i.endDate
            ? i.endDate instanceof Date
              ? (i.endDate = this._local_to_utc(this._zero_time(i.endDate)))
              : (i.endDate = g.parseDate(i.endDate, s, i.language))
            : (i.endDate = 1 / 0)),
      ...
      this.each(function () {
        ...
        if (!o) {
          ...
          if (h.is(".input-daterange") || D.inputs) {
            var v = { inputs: D.inputs || h.find("input").toArray() };
        ...
        if (
          ...
          for (l = 0, k = w.length; k > l; l++) {
            if (((p = parseInt(u[l], 10)), (h = w[l]), isNaN(p)))
              switch (h) {
                case "MM":
                  (D = t(f[n].months).filter(r)),
                    (p = t.inArray(D[0], f[n].months) + 1);
                  break;
                case "M":
                  (D = t(f[n].monthsShort).filter(r)),
                    (p = t.inArray(D[0], f[n].monthsShort) + 1);
              }
            v[h] = p;
          }
          var _, C;
          for (l = 0; l < m.length; l++)
            (C = m[l]),
              C in v &&
                !isNaN(v[C]) &&
                ((_ = new Date(a)), y[C](_, v[C]), isNaN(_) || (a = _));
        }
        return a;
      },
